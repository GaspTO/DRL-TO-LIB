...
class DDPG_HER(HER_Base, DDPG):
    """DDPG algorithm with hindsight transition replay"""
    agent_name = "DDPG-HER"
        ...
        while not self.done:
            ...
            if self.time_for_critic_and_actor_to_learn():
                for _ in range(self.hyperparameters["learning_updates_per_learning_session"]):
                    states, actions, rewards, next_states, dones = self.sample_from_HER_and_Ordinary_Buffer()  # Samples transitions from buffer
    ...
    def enough_transitions_to_learn_from(self):
        """Returns boolean indicating whether there are enough transitions to learn from and it is time to learn"""
