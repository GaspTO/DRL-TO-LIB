...
class Base_Agent(object):
    ...
    def __init__(self, config):
        ...
        self.action_mask_required = self.get_action_mask_required()
    ...
    def get_action_mask_required(self):
        if(self.environment_title in ["Gomoku"]):
            return True
        return False
    ...
    def get_output_shape(self):
        """ also adds batch dimension """
        return torch.tensor([self.action_size]) 
    ...
    def create_NN_through_NNbuilder(self,input_dim,output_size):
        return NNbuilder(config=self.config.hyperparameters,input_dim=input_dim,output_size=output_size)
