...
class DDQN_With_Prioritised_transition_Replay(DDQN):
    """A DQN agent with prioritised transition replay"""
    ...
    def learn(self):
        ...
        sampled_transitions, importance_sampling_weights = self.memory.sample()
        states, actions, rewards, next_states, dones = sampled_transitions
    ...
    def save_transition(self):
        """Saves the latest transition including the td_error"""
        max_td_error_in_transitions = self.memory.give_max_td_error() + 1e-9
        self.memory.add_transition(max_td_error_in_transitions, self.state, self.action, self.reward, self.next_state, self.done)
