...
class DAGGER(REINFORCE):
    ...
    def reset_game(self):
        ...
        self.episode_expert_probability_vectors = []
    ...
    def conduct_action(self):
        ...
        self.action, self.log_probabilities = self.pick_action_and_get_log_probs()
        #self.expert_probabilities = torch.tensor(self.mcts_rl(self.state))
        self.expert_probabilities = torch.tensor(self.mcts(self.state))
    ...
    def pick_action_and_get_log_probs(self,current_state=None):
        ...
        return action.item(), torch.log_softmax(action_values_logits,dim=1)[0]
    ...
    def save_update_information(self):
        ...
        self.episode_expert_probability_vectors.append(self.expert_probabilities)
        self.total_episode_score_so_far += self.reward

    def mcts_rl(self,state):
        #todo some things in here need config
        search = MCTS_RL_Agent(self.environment.environment,25,self.policy,self.device,exploration_weight=5.0)
        probs = search.get_play_probabilities(state)
        return probs
    ...
    def learn(self):
        ...
        log_values = torch.stack(self.episode_action_log_probability_vectors)
        targets = torch.stack(self.episode_expert_probability_vectors) 
