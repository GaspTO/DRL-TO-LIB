...
from agents.STI.Expansion_Strategy import Expansion_Strategy, One_Successor_Rollout, Network_One_Successor_Rollout, Network_Policy_Value
from agents.STI.Astar_minimax import Astar_minimax


Data = namedtuple('Data', ['observation', 'action','mask'])
Episode = namedtuple('Episode', ['observations', 'actions','masks'])
...
class ALPHAZERO(Learning_Agent):
    ...
    *            Main interface to be used by every implemented agent               
    ...
    def step(self):
        ...
        self.expert_action = self.try_expert(self.observation,25,1,1.0)
        ''' '''
    ...
    def pick_action(self,current_observation=None) -> tuple([int,dict]):
        ...
        self.policy.load_state(input_state)
        ''' policy '''
        policy_values_logits = self.policy.get_policy_values(False,input_mask)
        policy_values_softmax =  torch.softmax(policy_values_logits,dim=1)
        action = policy_values_softmax.argmax()
        ''' state value '''
        state_value = self.policy.get_state_value()
    ...
    *            Main interface to be used by every implemented agent               
    ...
    def try_expert(self,observation,n_rounds,k,exploration_weight):
        ...
        #tree_expansion = Network_Policy_Value(self.policy,self.device)
