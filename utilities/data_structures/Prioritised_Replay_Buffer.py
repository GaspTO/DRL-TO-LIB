...
class Prioritised_Replay_Buffer(Max_Heap, Deque):
    ...
    def separate_out_data_types(self, experiences):
        ...
        states = torch.tensor([e.value[self.indexes_in_node_value_tuple["state"]] for e in experiences]).float().to(self.device)
        actions = torch.tensor([[e.value[self.indexes_in_node_value_tuple["action"]]] for e in experiences]).float().to(self.device)
        rewards = torch.tensor([[e.value[self.indexes_in_node_value_tuple["reward"]]] for e in experiences]).float().to(self.device)
        next_states = torch.tensor([e.value[self.indexes_in_node_value_tuple["next_state"]] for e in experiences]).float().to(
            self.device)
        dones = torch.tensor([[int(e.value[self.indexes_in_node_value_tuple["done"]])] for e in experiences]).float().to(self.device)
